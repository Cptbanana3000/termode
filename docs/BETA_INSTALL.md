...
```text
Termode-v0.46-git-artifact-debug.apk
```
...
1. Copy `Termode-v0.46-git-artifact-debug.apk` to the device.
...
```sh
adb install -r Termode-v0.46-git-artifact-debug.apk
```
...
Node.js, npm, and Python are expected in v0.46 (planned, not installed).
...
```sh
cp build/app/outputs/flutter-apk/app-debug.apk Termode-v0.46-git-artifact-debug.apk
```

On Windows PowerShell:

```powershell
Copy-Item build\app\outputs\flutter-apk\app-debug.apk Termode-v0.46-git-artifact-debug.apk
```
