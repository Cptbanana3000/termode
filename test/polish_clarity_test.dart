...
void main() {
  ...
  group('Polish and Clarity Command Tests', () {
    ...
    test('whereami displays both VFS and native sandbox environments', () async {
      ...
      expect(result.output, contains('NOTE: Dart VFS, native runtime, and user-linked Android storage are currently separate.'));
    });
    ...
    test('runtime-help lists only native commands and warning message', () async {
      ...
      expect(result.isError, isFalse);
      expect(result.output, contains('WARNING: Native sandbox commands operate directly on physical storage.'));
    ...
    test('default help lists VFS commands in mobile-optimized layout', () async {
      final vfs = VirtualFileSystem();
      final commandService = CommandService(vfs, 'session_polish');

      final result = await commandService.execute('help');
      expect(result.isError, isFalse);
      expect(result.output, contains('Termode runs in a true native REAL PTY shell by default.'));
      expect(result.output, contains('whereami    - View active sandbox directories'));
      expect(result.output, contains('Termode VFS Commands:'));
      expect(result.output, contains('pwd         - Print VFS working directory'));
