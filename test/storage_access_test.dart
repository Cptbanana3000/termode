...
void main() {
  ...
  group('StorageAccessService and Commands Tests', () {
    ...
    setUp(() async {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(
        ...
        (MethodCall methodCall) async {
          if (mockForceRevoked) {
            throw PlatformException(code: 'PERMISSION_REVOKED', message: 'Permission revoked');
          }

          switch (methodCall.method) {
            case 'pickStorageFolder':
              mockLinkedUri = 'content://com.android.providers.downloads.documents/tree/raw%3A%2Fstorage%2Femulated%2F0%2FDownload%2Ftest';
            ...
            case 'listStorageFiles':
              if (mockLinkedUri == null) {
                throw PlatformException(code: 'NOT_LINKED', message: 'No linked folder');
              }
              return mockFileList;
            ...
            case 'createStorageDirectory':
              ...
              final folderName = methodCall.arguments['folderName'] as String;
            ...
            case 'getDiagnostics':
              return {
                ...
                'runtimePath': '/data/user/0/com.termode.termode/files/usr/bin:/system/bin',
              };
            case 'getPaths':
              return {
    ...
    test('storage-link and storage-status commands', () async {
      ...
      expect(statusResult1.output, contains('No folder is currently linked'));
      ...
      expect(statusResult2.output, contains('Linked storage folder: content://'));
    ...
    test('storage-unlink command resets permission', () async {
      ...
      expect(status.output, contains('Linked storage folder: content://'));

      // Unlink
      final unlinkResult = await commandService.execute('storage-unlink');
      expect(unlinkResult.output, contains('Storage link removed successfully'));

      // Verify status is back to unlinked
      status = await commandService.execute('storage-status');
      expect(status.output, contains('No folder is currently linked'));
    });
    ...
    test('storage read, write, and list operations', () async {
      ...
      // Write file
      final writeResult = await commandService.execute('storage-write notes.txt Buy groceries and code Flutter');
    ...
    test('storage-delete and storage-mkdir operations', () async {
      ...
      // Delete file
      final deleteResult = await commandService.execute('storage-delete file1.txt');
    ...
    test('support spaces in filenames using quotes and space joining', () async {
      final vfs = VirtualFileSystem();
      final commandService = CommandService(vfs, 'session_storage');
      ...
      final writeResult = await commandService.execute('storage-write "my test notes.txt" hello spaces');
      expect(writeResult.isError, isFalse);
      expect(writeResult.output, contains('my test notes.txt'));

      // Read with space (both quoted and unquoted)
      final readResultQuoted = await commandService.execute('storage-read "my test notes.txt"');
      expect(readResultQuoted.output, contains('hello spaces'));

      final readResultUnquoted = await commandService.execute('storage-read my test notes.txt');
      expect(readResultUnquoted.output, contains('hello spaces'));

      // Delete with space (unquoted)
      final deleteResult = await commandService.execute('storage-delete my test notes.txt');
      expect(deleteResult.isError, isFalse);
      expect(deleteResult.output, contains('Deleted file "my test notes.txt"'));
    });
    ...
    test('storage-test diagnostics runs complete verification loop', () async {
      ...
      expect(testResultPass.output, contains('Checking storage status... PASS'));
      ...
      expect(testResultPass.output, contains('Deleting temporary test file... PASS'));
    });
    ...
    test('whereami displays display name and URI of linked storage', () async {
      ...
      expect(whereamiResult.output, contains('Status: LINKED (Name: MyTestFolder)'));
    ...
    test('error mappings return user friendly descriptions', () async {
      ...
      final readErr = await commandService.execute('storage-read missing_file.txt');
      ...
      expect(statusErr.output, contains('Permission revoked or folder access denied'));
    });
    ...
    test('storage-help describes commands and limitation details', () async {
      ...
      expect(helpResult.output, contains('=== Termode User-Approved Storage Help ==='));
      ...
      expect(helpResult.output, contains('Android security limits direct storage access'));
    });
  });
}
