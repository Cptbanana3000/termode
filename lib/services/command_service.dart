...
class CommandService {
  ...
  String _toolchainPlanOutput() {
    return '=== Toolchain Plan ===\n'
        ...
        ' 11. Git source/dependency preparation (v0.53)\n'
        ...
        ' 17. Git build fixes (v0.59)\n'
        ' 18. Git build host strategy (v0.60)\n'
        ' 19. Git arm64 build under Git Bash (v0.61)\n'
        ' 20. Git Bash build fixes (v0.62)\n'
        ' 21. Node.js/npm/Python remain planned\n'
  ...
  String _runtimeInstallListOutput() {
    final gitState = RuntimeBinaryPackageService().gitArtifactAvailable()
        ...
        : 'pipeline ready (artifact not bundled)';
  ...
  String _runtimeInstallPlanOutput(String tool) {
    ...
    if (key == 'git') {
      ...
      return '=== Runtime Install Plan: Git ===\n'
          ...
          'Minimal target: ${GitBuildService.minimalTarget}\n'
          'Future/supported steps:\n'
          '1. Resolve Perl and acquire trusted Git/dependency sources.\n'
  ...
  Future<String> _gitInfoOutput() async {
    ...
    return '=== Git Info ===\n'
        ...
        'Installed: ${installed ? 'yes' : 'no'}\n'
        'Selected source target: ${GitBuildService.selectedGitVersion}\n'
        'Minimal first target: ${GitBuildService.minimalTarget}\n'
  ...
  String _gitPlanOutput(bool artifactAvailable) {
    return '=== Git Support Plan ===\n'
        'Selected Git version: ${GitBuildService.selectedGitVersion}\n'
        '1. Resolve Perl and stage trusted source/dependency inputs.\n'
        '2. Verify ABI.\n'
        '3. Validate Git package manifest.\n'
        '4. Install files into TERMODE_PREFIX.\n'
        '5. Register git shim.\n'
        '6. Run git --version.\n'
        '7. Run git-doctor.\n'
        '8. Test git init/status in workspace.\n'
  ...
  Future<String> _gitWorkspaceSmokePlanOutput() async {
    final installed = await RuntimeBinaryPackageService().gitInstalled();
    final header = installed
        ? 'Git is installed. Run manually when ready:'
        : 'Blocked: missing trusted Git artifact.';
    return '=== Git Workspace Smoke Plan ===\n'
        '$header\n'
        'workspace-init gittests\n'
        'workspace-cd gittests\n'
        'git --version\n'
  ...
  String _gitArtifactHelpOutput() {
    return '=== Git Artifact ===\n'
        ...
        '  git-artifact manifest  - show manifest summary if present\n'
        '  git-artifact verify    - verify a present artifact (no install)\n'
        '  git-artifact doctor    - full artifact readiness check\n'
        ...
        '  git-artifact production-status - trusted build production status\n'
        ...
        '  git-workspace-smoke-plan   - manual workspace smoke plan';
  }

  Future<String> _gitArtifactStatusOutput() async {
    final a = await RuntimeArtifactRegistryService().gitArtifactStatus();
    return '=== Git Artifact Status ===\n'
        'Current ABI: ${a.abi}\n'
        'Artifact available: ${a.available ? 'yes' : 'no'}\n'
        'Installable: ${a.installable ? 'yes' : 'no'}\n'
        'Template present: ${a.templatePresent ? 'yes' : 'no'}\n'
        'Bundled manifest: ${a.manifestPresent ? 'yes' : 'no'}\n'
        'Reason: ${a.reason}\n'
        'Source: ${a.source}\n'
        'Overall: ${a.status}';
  }

  Future<String> _gitArtifactInfoOutput() async {
    final a = await RuntimeArtifactRegistryService().gitArtifactStatus();
    return '=== Git Artifact Info ===\n'
        'Installing real Git requires a verified, ABI-matched Git package\n'
        'artifact (a bundled/trusted runtime package). No internet download\n'
        'or arbitrary archive import is enabled.\n'
        'This build has it: ${a.available ? 'yes' : 'no'}\n'
  ...
  Future<String> _gitArtifactManifestOutput() async {
    ...
    if (manifest == null) {
      ...
      return 'Git artifact manifest is not available in this build.\n'
          'Template: ${registry.gitTemplateExists() ? 'present' : 'missing'}\n'
          'Template path: ${RuntimeArtifactRegistryService.gitTemplatePath}\n'
          'Template validation: ${templateErrors.isEmpty ? 'OK' : templateErrors.first}\n'
          'Installable: no';
    }
    return '=== Git Artifact Manifest ===\n'
        'Name: ${manifest['name']}\n'
  ...
  Future<String> _gitArtifactVerifyOutput() async {
    ...
    return '=== Git Artifact Verify ===\n'
        'Artifact: present\n'
        'Manifest/ABI/checksum: ${a.installable ? 'OK' : a.reason}\n'
        'Note: verify does not install.\n'
        'Overall: ${a.status}';
  }
  ...
  Future<String> _gitArtifactDoctorOutput() async {
    ...
    return '=== Git Artifact Doctor ===\n'
        ...
        'Registry: bundled (${registry.bundledGitArtifactExists() ? 'present' : 'empty'})\n'
        'Artifact: ${a.status}\n'
        'Template: ${a.templatePresent ? 'present' : 'missing'}\n'
        'Manifest: ${manifest == null ? 'missing' : 'present'}\n'
        'Checksum: ${a.available ? (a.installable ? 'OK' : 'CHECK') : 'n/a'}\n'
        'Install readiness: ${a.installable ? 'installable' : 'not installable (${a.reason})'}\n'
        'Note: a missing Git artifact is expected; this is not an app failure.\n'
        'Overall: ${a.status}';
  }
  ...
  String _gitArtifactPipelineOutput() {
    return '=== Git Artifact Pipeline ===\n'
        ...
        '1. Acquire/build Git ${GitBuildService.selectedGitVersion} from a trusted, documented source.\n'
        ...
        'Template: tools/runtime-artifacts/git/manifest.template.json.\n'
        '4. Verify SHA-256 checksums for every file.\n'
  ...
  String _gitArtifactRequirementsOutput() {
    return '=== Git Artifact Requirements ===\n'
        'Required manifest fields:\n'
        'name, version, kind, abi, command, entrypoint, files, sha256,\n'
        'source, source_url, build_method, license, trusted_by,\n'
        'verification_command, smoke_tests, dependencies, created_at\n\n'
        'Rules:\n'
        '* name must be git\n'
        '* kind must be native-tool\n'
        '* command must be git\n'
        '* entrypoint/files must be safe relative paths\n'
        '* source must be termode-built or termode-vendored\n'
        '* every file needs a 64-character SHA-256 checksum\n'
        '* TEMPLATE_ONLY is not installable\n'
        '* zero-byte/all-zero checksum payload entries are rejected';
  }

  String _gitArtifactSourcesOutput() {
    return '=== Git Artifact Sources ===\n'
        'Allowed:\n'
        '* termode-built\n'
        '* termode-vendored\n\n'
        'Rejected:\n'
        '* random internet binaries\n'
        '* copied Termux binaries\n'
        '* runtime downloads\n'
        '* arbitrary user archives\n'
        '* manifests with unsafe paths or missing checksums';
  }

  Future<String> _gitArtifactNextOutput() async {
    final a = await RuntimeArtifactRegistryService().gitArtifactStatus();
    return '=== Git Artifact Next ===\n'
        'Current state: ${a.status}\n'
  ...
  String _gitArtifactBundlePlanOutput() {
    return '=== Git Bundle Plan ===\n'
        '1. Produce trusted Git build for arm64-v8a.\n'
  ...
  Future<String> _gitExecProbeOutput() async {
    ...
    final out = result.stdout.trim().isNotEmpty
        ? result.stdout.trim()
        : result.stderr.trim();
    final healthy = result.exitCode == 0 && out.toLowerCase().contains('git');
    return '=== Git Exec Probe ===\n'
        'Exit code: ${result.exitCode}\n'
        'Output: $out\n'
  ...
  Future<CommandResult> execute(String input) async {
    ...
    switch (command) {
      ...
      case 'git-build-host-strategy':
        return CommandResult(output: GitBuildService().hostStrategy());
      ...
      case 'git-perl-status':
        return CommandResult(output: GitBuildService().perlStatus());

      case 'git-source-version':
        return CommandResult(output: GitBuildService().sourceVersion());

      case 'git-source-checklist':
        return CommandResult(output: GitBuildService().sourceChecklist());

      case 'git-deps-minimal':
        return CommandResult(output: GitBuildService().dependenciesMinimal());

      case 'git-build-next-steps':
        return CommandResult(output: GitBuildService().buildNextSteps());

      case 'git-build-readiness':
      ...
      case 'git-artifact':
        ...
        switch (sub) {
          case 'help':
            return CommandResult(output: _gitArtifactHelpOutput());
          case 'status':
            return CommandResult(output: await _gitArtifactStatusOutput());
          case 'info':
            return CommandResult(output: await _gitArtifactInfoOutput());
          case 'manifest':
            return CommandResult(output: await _gitArtifactManifestOutput());
          case 'verify':
            return CommandResult(output: await _gitArtifactVerifyOutput());
          case 'doctor':
            return CommandResult(output: await _gitArtifactDoctorOutput());
          case 'pipeline':
            return CommandResult(output: _gitArtifactPipelineOutput());
          case 'requirements':
            return CommandResult(output: _gitArtifactRequirementsOutput());
          case 'sources':
            return CommandResult(output: _gitArtifactSourcesOutput());
          case 'production-status':
            return CommandResult(
              output: await _gitArtifactProductionStatusOutput(),
            );
          case 'next':
          ...
          default:
            return CommandResult(
              output:
                  ...
                  'Usage: git-artifact <status|info|manifest|verify|doctor|pipeline|requirements|sources|production-status|next|bundle-status|bundle-plan|bundle-check|smoke-plan>',
              isError: true,
            );
        }

      case 'git-exec-probe':
        final output = await _gitExecProbeOutput();
        return CommandResult(
          output: output,
          isError: output.contains('Overall: UNHEALTHY'),
        );
      ...
      case 'git-smoke-test':
        final output = await _gitSmokeTestOutput();
        return CommandResult(
          output: output,
          isError: output.contains('Overall: UNHEALTHY'),
        );
